...
const aiChatModule: AssistantModule = {
  id: 'ai-chat',
  ...
  functions: {
    chat: async (args: { 
    ...
    }, ctx: ModuleContext): Promise<string> => {
      const now = Date.now();
      ...
      ctx.log.debug('[ai-chat] send prompt len=%d model=%s', args.prompt.length, args.model || 'default');
      ...
      try {
              ...
              try {
                ...
                if (chunk.tool_calls) {
                  ...
                  for (const tool of chunk.tool_calls) {
                    ctx.log.debug('[ai-chat] tool call: %s', tool.name);
                    // Parse module and function from tool name (format: "module.function")
                    const [moduleName, functionName] = tool.name.split('.');
                    if (moduleName && functionName) {
                      // Note: ctx.invoke is synchronous, no await needed
                      try {
                        ctx.invoke(moduleName, functionName, tool.arguments);
                      } catch (err) {
                        ctx.log.error('[ai-chat] tool call failed: %s - %o', tool.name, err);
                      }
                    }
                  }
                }
              } catch (err) {
                ctx.log.error('[ai-chat] failed to parse chunk: %o', err);
              }
            }
          }
        });
        ...
        ctx.log.error('[ai-chat] stream error %o', error);
        throw error;
      }
      ...
      ctx.log.info('[ai-chat] streamed %d chars', assistantContent.length);
      return assistantContent;
    },
    
    'get-history': async (_args: unknown, ctx: ModuleContext): Promise<Array<{ role: string; content: string; updated_at: number }>> => {
  ...
  async init(ctx: ModuleContext) {
    ...
    ctx.log.info('[ai-chat] module ready');
  }
};
